...
<div class="admin-modalidades">
    ...
    <div class="action-bar">
        ...
        <div class="search-wrapper">
            <input type="text" class="search-input-admin" id="buscarModalidad" placeholder="Buscar modalidad..."
                onkeyup="filtrarModalidades()">
        </div>
    </div>
    ...
    <div class="modalidades-grid" id="modalidadesGrid">
        ...
        <?php else: ?>
            <?php foreach ($modalidades as $index => $m): ?>
                <div class="modalidad-card" data-id="<?php echo $m['id']; ?>"
                        ...
                        <div class="card-body-modalidad">
                            ...
                            <div class="stats-row">
                                ...
                                <span class="fecha-badge">
                                    ...
                                    <?php echo isset($m['created_at']) ? date('d/m/Y', strtotime($m['created_at'])) : 'N/A'; ?>
                                </span>
                            </div>
                        ...
                        <div class="card-footer-modalidad">
                            <div class="action-group">
                                ...
                                <button class="btn-action-card btn-edit-card"
                                    onclick="editarModalidad(<?php echo $m['id']; ?>, '<?php echo htmlspecialchars(addslashes($m['nombre'])); ?>', '<?php echo htmlspecialchars(addslashes($m['descripcion'] ?? '')); ?>')">
                                    <i class="bi bi-pencil"></i> Editar
                                </button>
                                <button class="btn-action-card btn-delete-card"
                                    onclick="eliminarModalidad(<?php echo $m['id']; ?>, '<?php echo htmlspecialchars(addslashes($m['nombre'])); ?>', <?php echo $m['carreras_count']; ?>)">
                ...
                <div class="modal-body modal-body-admin">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="form-hint">
                            ...
                            Máximo 100 caracteres - Mínimo 3 caracteres
                        </small>
                    </div>
...
<script>
    ...
    let modalVerModalidad, modalModalidad;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        modalModalidad = new bootstrap.Modal(document.getElementById('modalModalidad'));
    });
        ...
        fetch(`index.php?action=modalidad-ver&id=${id}`)
            ...
            .then(data => {
                ...
                if (data.carreras && data.carreras.length > 0) {
                    carrerasHtml = '<table class="carreras-table" style="width:100%; border-collapse: collapse;">';
                    data.carreras.forEach(c => {
                        carrerasHtml += `
                        <tr style="border-bottom: 1px solid #edf2f7;">
                            <td style="padding: 10px; width: 40%;"><strong>${c.nombre}</strong></td>
                            <td style="padding: 10px; width: 40%;">${c.universidad_nombre}</td>
                            <td style="padding: 10px; width: 20%;"><span class="badge-region" style="background: linear-gradient(135deg, #667eea15, #764ba215); color: #667eea; padding: 5px 12px; border-radius: 50px; font-size: 0.8rem;">${c.region_nombre}</span></td>
                        </tr>
                    `;
                    });
                    carrerasHtml += '</table>';
                ...
                const fechaCreacion = data.created_at ? new Date(data.created_at).toLocaleDateString() : 'No disponible';
                ...
                <div class="text-center mb-4">
                    <div class="detail-icon" style="width: 80px; height: 80px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                        <i class="bi bi-grid-3x3-gap-fill" style="font-size: 2.5rem; color: white;"></i>
                    </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <div class="info-card-detail" style="background: #f8fafc; padding: 20px; border-radius: 15px;">
                            ...
                            <p><i class="bi bi-calendar me-2"></i> <strong>Fecha Creación:</strong> ${fechaCreacion}</p>
    ...
    function filtrarModalidades() {
        ...
        if (visibleCount === 0 && busqueda !== '') {
            if (!mensajeExistente) {
                ...
                msg.innerHTML = `
                <div class="no-results" style="text-align: center; padding: 40px; background: #f8fafc; border-radius: 15px; margin: 20px 0;">
